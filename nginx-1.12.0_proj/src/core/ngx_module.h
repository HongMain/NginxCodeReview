...
/*
 * nginx是完全模块化的，所有的组件都是模块（ngx_module_t类型）
 * 从而实现了nginx的高度松耦合。同时，我们在进行nginx模块开发时，也离不开这个数据结构。
 */
struct ngx_module_s {
    ngx_uint_t            ctx_index;        // 在具体类型模块（http、event等）的全局配置结构数组的下标
    
    /*
     * 该模块在ngx_modules数组中的下标
     * 同时nginx把所有的core module的配置结构存放到ngx_cycle的conf_ctx数组中，
     * index也是该模块的配置结构在ngx_cycle->conf_ctx数组中的下标。
     */
    ngx_uint_t            index;
